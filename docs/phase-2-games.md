# Fase 2: Colección de Videojuegos — Backend
...
| GET | `/api/games?platform=pc` | Filtrar por plataforma |
...
- [ ] Crear `GameSearchService.java` en `application/service/`:
  ...
  - Delega a `ExternalGameCatalogClient`
...
- [ ] Crear `GameController.java` en `infrastructure/adapter/in/web/`:
  ...
  - `GET /api/games/search?name={query}` — buscar en FreeToGame
...
- [ ] Registrar rutas en `GameController` (no necesita cambios en App.jsx si es backend)
...
- [ ] El endpoint `GET /api/games/search?name=overwatch` devuelve resultados de FreeToGame
...
- Si en el futuro se quiere una API más completa, se puede añadir IGDB/RAWG como segundo `ExternalGameCatalogClient`
