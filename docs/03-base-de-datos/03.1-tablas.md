...
Las imágenes subidas **no** se guardan en MongoDB: viven en el filesystem local
(`app.image.storage.path`, `./uploads/images`) y se sirven como estáticos en
`/api/v1/images/**`. En la base de datos solo se almacena la URL pública en el
campo de imagen de cada entidad (`frontpage`, `thumbnailUrl`, `posterUrl`, etc.).
